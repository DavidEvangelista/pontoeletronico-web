...
class UserJWTController(
    ...
    fun authorize(@Valid @RequestBody loginVM: LoginVM): ResponseEntity<JWTToken> {
        ...
        val user: Optional<User>? = loginVM.username?.let { userService.getUserWithAuthoritiesByLogin(it) }
        val jwt = user?.get()?.id?.let { tokenProvider.createToken(authentication, rememberMe, it) }
